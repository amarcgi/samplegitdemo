...
public class LoginServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if(personId == null){    
            ...
            try {
                Person person = personDao.login(request.getParameter("email"), request.getParameter("password"));
                if(person == null) {
                    ...
                    session.setAttribute("personName", person.getFirstName() + " " + person.getLastName());
                    response.sendRedirect("wall?ownerId=" + person.getId());
                }
            } finally {            
                out.close();
            }
